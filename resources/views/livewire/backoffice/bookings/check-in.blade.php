<div class="space-y-4">
    ...
    @else
        ...
        <form wire:submit.prevent="confirm" class="space-y-4">
            <div>
                <label class="block text-sm font-medium text-gray-700">Actual check-in *</label>
                <input
                    type="datetime-local"
                    wire:model.defer="form.actual_check_in_at"
                    class="mt-1 w-full rounded border px-3 py-2 focus:outline-none focus:ring focus:ring-blue-200"
                >
                @error('form.actual_check_in_at') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>
            ...
            <div class="flex items-center justify-end gap-2">
                <button
                    ...
                    wire:click="$dispatch('modal-close', { id: 'booking-check-in' })"
                >
                    Cancel
                </button>
                ...
                <button
                    type="submit"
                    class="rounded bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700 disabled:opacity-70"
